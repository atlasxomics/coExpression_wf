...
inp_genes <- read.csv(args[4], header = F)$V1
# inp_genes
#=============================================================================
# check gene list
...
for (reduction in reductions){
  if(reduction=='UMAP'){
    pbmc_2 <- pbmc
  }else{pbmc_2 <- pbmc[,which(pbmc@meta.data$Sample==reduction)]}

ggOut <- FeaturePlot(pbmc_2,
                     reduction = reduction,
...
for (i in seq_along(reductions)) {
  if(reductions[i]=='UMAP'){
    pbmc_2 <- pbmc
  }else{pbmc_2 <- pbmc[,which(pbmc@meta.data$Sample==reductions[i])]}
